...
build_sysimg(;force=false) = build_sysimg(joinpath(dirname(Sys.dlpath("libjulia")),"sys"), force=force)

# Build a system image binary at sysimg_path.dlext.  If a system image is already loaded, error out, or continue if force = true
function build_sysimg(sysimg_path; force=false)
    ...
    run(`$julia --build $sys0_path sysimg.jl`)

    # Bootstrap off of that to create sys.{ji,o}
    println("Building sys.o...")
    run(`$julia --build $sysimg_path -J $sys0_path.ji -f sysimg.jl`)

    # Link sys.o into sys.$(dlext)
    FLAGS = ["-shared", "-fPIC", "-L$julia_libdir"]
    if OS_NAME == :Darwin
    ...
    run(`$cxx $FLAGS -o $sysimg_path.$(Sys.dlext) $sysimg_path.o`)
end

# Search for a C++ compiler to link sys.o into sys.dl_ext.  Honor CXX environment variable, otherwise search for something we know works
function find_cxx_compiler()
    if haskey( ENV, "CXX" )
        if !success(`$(ENV["CXX"]) --version`)
            warn("Using compiler override $(ENV["CXX"]), but unable to run `$(ENV["CXX"]) --version`")
        end
        return ENV["CXX"]
    end

    poss_compilers = ["clang++", "g++"]

    for compiler in poss_compilers
        try
            if success(`$compiler --version`)
                return compiler
            end
        end
    end

    error( "No supported compilers found (tried $(join(poss_compilers, ", "))), override with CXX environment variable!" )
end